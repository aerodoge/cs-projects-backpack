...
type TradingConfig struct {
	USDTAmount int64 `mapstructure:"usdt_amount"` // 每次交易的USDT数量
...
func (c *Config) Validate() error {
	if c.Lighter.APIKey == "" {
		return fmt.Errorf("lighter.api_key is required")
	}
	if c.Lighter.SecretKey == "" {
		return fmt.Errorf("lighter.secret_key is required")
	}
	if c.Lighter.PrivateKey == "" {
		return fmt.Errorf("lighter.private_key is required")
	}
